...
export function initFilter() {
  // 1. ГОЛОВНІ ЕЛЕМЕНТИ (Потрібні тільки для каталогу)
  ...
  // ГОЛОВНИЙ ЗАХИСТ: Якщо базових елементів каталогу немає на цій сторінці, модуль зупиняє роботу
  if (!minSlider || !maxSlider || !productList) return;
  ...
  function handleTextInput() {
    ...
    if (minPrice < 0) { minPrice = 0; inputMin.value = 0; }
    if (minPrice > parseInt(minSlider.max)) { minPrice = parseInt(minSlider.max); inputMin.value = minSlider.max; }
    if (maxPrice < 0) { maxPrice = 0; inputMax.value = 0; }
    if (maxPrice > 1000) { maxPrice = 1000; inputMax.value = 1000; }
